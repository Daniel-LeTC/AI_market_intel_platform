...
def render_mass_mode(selected_asin):
    ...
    if base_info.empty:
        # Fallback to product_parents if not yet in products table
        base_info = query_df(
            "SELECT brand, title, category, niche FROM product_parents WHERE parent_asin = ?", [selected_asin]
        )
        my_line = "N/A"
        my_ratings = 0
    else:
    ...
    with f_c1:
        ...
        selected_cat = st.selectbox(
            "Lọc theo Category:",
            ["Tất cả"] + unique_cats,
            index=unique_cats.index(my_cat) + 1 if my_cat in unique_cats else 0,
        )
    ...
    with f_c2:
        ...
        # FIX: Unnest the aggregated niche strings to get a clean list of individual niches
        raw_niches = niche_df["niche"].dropna().unique()
        all_individual_niches = set()
        for rn in raw_niches:
            for n in str(rn).split(","):
                n_clean = n.strip()
                if n_clean:
                    all_individual_niches.add(n_clean)

        unique_niches = sorted(list(all_individual_niches))

        # Check if current product's niche is in the list for index selection
        # (Since my_niche might be a list, we just default to index 0 or match first)
        selected_niche = st.selectbox("Lọc theo Niche:", ["Tất cả"] + unique_niches)
    ...
    if selected_niche != "Tất cả":
        # Match if the selected_niche exists anywhere in the parent's aggregated niche string
    ...
    # SAFE DEFAULT LOGIC: Prevent crash if selected_asin is filtered out
    safe_default = []
    if selected_asin in dynamic_parent_list:
        safe_default.append(selected_asin)

    # Add auto candidates that are present in the filtered list
    safe_default.extend([c for c in auto_candidates if c in dynamic_parent_list])
    # Remove potential duplicates while preserving order
    safe_default = list(dict.fromkeys(safe_default))

    st.markdown("##### 🤝 Tùy chỉnh danh sách so sánh")
    selected_list = st.multiselect(
        "Chọn các ASIN đối thủ để đưa vào bản đồ nhiệt:",
        options=dynamic_parent_list,
        default=safe_default,
        format_func=lambda x: f"{x} - {str(dynamic_parent_map.get(x, 'Unknown Brand'))[:15]}...",
        key=f"mass_sel_list_v4_{selected_asin}",
    )
