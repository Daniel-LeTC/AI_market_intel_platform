...
@st.cache_resource
def get_connection():
    """Ket noi DB mot lan va dung chung"""
...
if selected_asin:
    ...
    dna_query = f"""
        ...
        FROM products 
        WHERE asin = '{selected_asin}' OR parent_asin = '{selected_asin}' 
        LIMIT 1
    """
    dna = conn.execute(dna_query).df()
    
    # Lay Real-time variations tu Review DB
    real_vars_count = conn.execute(f"SELECT COUNT(DISTINCT child_asin) FROM reviews WHERE parent_asin = '{selected_asin}'").fetchone()[0]
    ...
    if view_mode == "🏠 Home Dashboard":
        ...
        kpi_query = f"""
            ...
            FROM reviews
            WHERE parent_asin = '{selected_asin}'
        """
        ...
        tag_count_query = f"SELECT COUNT(*) FROM review_tags WHERE parent_asin = '{selected_asin}'"
        tag_count = conn.execute(tag_count_query).fetchone()[0]
        ...
        if tag_count > 0:
            ...
            with ai_col1:
                ...
                pain_query = f"""
                    ...
                    WHERE rt.parent_asin = '{selected_asin}' AND rt.sentiment = 'Negative'
                    GROUP BY 1
                    ORDER BY 2 DESC
                    LIMIT 5
                """
                df_pain = conn.execute(pain_query).df()
            ...
            with ai_col2:
                ...
                aspect_query = f"""
                    ...
                    FROM review_tags rt
                    LEFT JOIN aspect_mapping am ON rt.aspect = am.raw_aspect
                    WHERE rt.parent_asin = '{selected_asin}'
                    GROUP BY 1
                    HAVING (positive + negative) > 1 
                    ORDER BY (positive + negative) DESC
                    LIMIT 10
                """
                df_aspect = conn.execute(aspect_query).df()
        ...
        # Lay danh sach cac ASIN co data AI Tags (de so sanh cho co y nghia)
        ai_asins_query = """
            SELECT DISTINCT parent_asin 
            FROM review_tags 
            WHERE parent_asin != ?
        """
        ai_asins = conn.execute(ai_asins_query, [selected_asin]).df()['parent_asin'].tolist()
        ...
        if ai_asins:
            ...
            if challenger_asin:
                ...
                # Query lay het stats cua ca 2 thang (De dung chung cho ca Section)
                battle_query = f"""
                    WITH normalized AS (
                        SELECT 
                            rt.parent_asin,
                ...
                """
                df_all = conn.execute(battle_query).df()
                ...
                with battle_row_top_c1:
                    ...
                    if not df_all.empty:
                        ...
                        shared_aspects = aspects_selected.intersection(aspects_challenger)
                        
                        df_shared = df_all[df_all['aspect'].isin(shared_aspects)]
                        if not df_shared.empty:
                            ...
                            fig_battle.update_layout(margin=dict(l=0, r=0, t=30, b=0))
                ...
                with battle_row_top_c2:
                    ...
                    def get_top_issues(asin):
                        q = f"""
                            ...
                            FROM review_tags rt
                            LEFT JOIN aspect_mapping am ON rt.aspect = am.raw_aspect
                            WHERE rt.parent_asin = '{asin}' AND rt.sentiment = 'Negative' 
                ...
                # 2. Bottom Row: Exclusive Features
                st.markdown("---")
        ...
        with c1:
            ...
            trend_query = f"SELECT DATE_TRUNC('month', review_date) as month, AVG(rating_score) as avg_score FROM reviews WHERE parent_asin = '{selected_asin}' GROUP BY 1 ORDER BY 1"
            df_trend = conn.execute(trend_query).df()
        ...
        with c2:
            ...
            df_dist = conn.execute(f"SELECT rating_score, COUNT(*) as count FROM reviews WHERE parent_asin = '{selected_asin}' GROUP BY 1").df()
        ...
        base_query = f"SELECT review_date, rating_score, title, text, variation_text, is_verified FROM reviews WHERE parent_asin = '{selected_asin}'"
        ...
        df_reviews = conn.execute(base_query).df()
        st.dataframe(
            df_reviews, 
            column_config={
